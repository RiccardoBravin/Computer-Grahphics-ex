...
vec3 BRDF(vec3 V, vec3 N, vec3 L, vec3 Md, vec3 Ms, float gamma) {
	...
	vec3 diffuse = Md * clamp(dot(L, N), 0, 1); //diffuse term

	vec3 H = normalize(L + V); //half vector
	vec3 specular = Ms * pow(clamp(dot(N, H) , 0, 1), gamma); //specular term
...
void main() {
	...
	vec3 lightColor = gubo.lightColor.xyz * 
					  ...
					  clamp((dot(lightDir,gubo.lightDir) - cosout)/(cosin-cosout),0,1);
	...
	vec3 DiffSpec = BRDF(V, N, L, MD, MS, 1000);
	...
	//result computation
	outColor = vec4(clamp(/*DiffSpec * lightColor*/ + MA * Ambient + ME, 0.0, 1.0), 1.0f); // output color
